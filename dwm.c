		...
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				if (!ISVISIBLE(c) || c == m->sel)
					continue;
				tw = TEXTW(c->name) + (c->icon ? c->icw + ICONSPACING : 0);
				if(tw < mw)
					ew += (mw - tw);
				else
					i++;
            ...
            if (c->icon) drw_pic(drw, x + lrpad / 2, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, c->icon);
				if (c->isfloating)
...
runAutostart()
{
   system("cd ~/bin; ./dwm_autostart.sh &");
}
